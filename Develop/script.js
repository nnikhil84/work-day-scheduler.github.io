...
window.onload = function () {
  const currentTimeblocks = obtainTimeblocks();
  const currentTime = moment();

  currentDateDisplay(currentTime);
  timeRows(currentTime);

  document
    .querySelector(".container")
    .addEventListener("click", function (event) {
      containerClick(event, currentTimeblocks);
    });
  textTimeblocks(currentTimeblocks);
};

function obtainTimeblocks() {
...
// Function for displaying all timeblock rows
function timeRows(currentTime) {
...
function containerClick(event, timeblockList) {
...
function textTimeblocks(timeblockList) {
  if (timeblockList.length === 0) {
    return;
  } else {
